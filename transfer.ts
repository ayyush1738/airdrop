...
const turbin3Wallet = address(
    '3AM4N2hitJxtkZbN3newYE62Yq2p33DzM7VUsrnXmF1a'
);
...
// Create and prepare transaction message
...
const transferInstructionFull = getTransferSolInstruction({
    source: keypair,
    destination: turbin3Wallet,
    amount: lamports(sendAmount)
});

const transactionMessageFull = pipe(
    createTransactionMessage({ version: 0 }),
    tx => setTransactionMessageFeePayerSigner(keypair, tx),
    tx => setTransactionMessageLifetimeUsingBlockhash(latestBlockhash, tx),
    tx => appendTransactionMessageInstructions([transferInstructionFull], tx)
);

const signedTransactionFull = await signTransactionMessageWithSigners(transactionMessageFull);
assertIsTransactionWithinSizeLimit(signedTransactionFull);

const sendAndConfirmTransactionFull = sendAndConfirmTransactionFactory({
    rpc,
    rpcSubscriptions
});

try {
    await sendAndConfirmTransactionFull(signedTransactionFull, { commitment: 'confirmed' });
    const signature = getSignatureFromTransaction(signedTransactionFull);
    console.log(`Success! Check out your TX here: https://explorer.solana.com/tx/${signature}?cluster=devnet`);
} catch (e) {
    console.error('Transfer failed:', e);
}
